      ...
      <div className="flex h-[calc(100vh-52px)]">
        ...
        <div className="w-80 border-r border-white/7 flex flex-col flex-shrink-0">
          <div className="flex items-center justify-between px-5 py-4 border-b border-white/7">
            <div className="text-[9px] tracking-[.25em] uppercase text-[#b08d57]">Campaigns</div>
            <button onClick={() => setCreating(true)}
              className="text-[9px] tracking-[.16em] uppercase text-[#52504c] hover:text-[#b08d57] transition-colors">
              + New
            </button>
          </div>
            ...
            {loading ? (
              <div className="flex items-center justify-center h-24">
                <div className="flex gap-1">
                  {[0,1,2].map(i => <div key={i} className="w-1 h-1 rounded-full bg-[#b08d57] animate-pulse" style={{ animationDelay: `${i*0.2}s` }} />)}
                </div>
              </div>
            ) : automations.length === 0 ? (
              <div className="px-5 py-8 text-center">
                <div className="text-[10px] tracking-[.08em] text-[#52504c] mb-3">No campaigns yet</div>
                <button onClick={() => setCreating(true)} className="text-[9px] tracking-[.16em] uppercase text-[#b08d57] hover:opacity-70 transition-opacity">
                  Create your first →
                </button>
              </div>
            ) : automations.map(a => (
              <div key={a.id}
                ...
                className={`px-5 py-4 border-b border-white/5 cursor-pointer hover:bg-white/3 transition-colors ${selectedId === a.id ? 'bg-white/5 border-l-2 border-l-[#b08d57]' : ''}`}>
                ...
                <div className="flex items-center gap-3 mt-1.5">
                  {a.trigger_keyword && (
                    <span className="text-[9px] tracking-[.1em] text-[#52504c] bg-white/5 px-1.5 py-0.5">"{a.trigger_keyword}"</span>
                  )}
                  <span className="text-[9px] tracking-[.06em] text-[#52504c]">{a.sent_count || 0} sent · {a.lead_count || 0} leads</span>
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="flex-1 overflow-y-auto">
          ...
          {creating && (
            <div className="p-8 border-b border-white/7">
              <div className="flex items-center justify-between mb-6">
                <div className="text-[10px] tracking-[.22em] uppercase text-[#b08d57]">New campaign</div>
                <button onClick={() => setCreating(false)} className="text-[#52504c] hover:text-[#8a8780] transition-colors text-sm">×</button>
              </div>
              <div className="grid grid-cols-2 gap-4 mb-4">
                <div className="col-span-2">
                  <label className="block text-[9px] tracking-[.2em] uppercase text-[#52504c] mb-2">Campaign name</label>
                  <input value={form.campaign_name} onChange={e => setForm(p => ({ ...p, campaign_name: e.target.value }))}
                    placeholder="EP Launch — Free Download"
                    className="w-full bg-[#0e0d0b] border border-white/10 text-[#f0ebe2] font-mono text-[11px] px-3 py-2.5 outline-none focus:border-[#b08d57] transition-colors placeholder-[#2e2c29]" />
                </div>
                <div>
                  <label className="block text-[9px] tracking-[.2em] uppercase text-[#52504c] mb-2">
                    Trigger keyword <span className="text-[#2e2c29] normal-case">leave blank = any comment</span>
                  </label>
                ...
                <div>
                  <label className="block text-[9px] tracking-[.2em] uppercase text-[#52504c] mb-2">Reward type</label>
                  <select value={form.reward_type} onChange={e => setForm(p => ({ ...p, reward_type: e.target.value }))}
                    className="w-full bg-[#0e0d0b] border border-white/10 text-[#f0ebe2] font-mono text-[11px] px-3 py-2.5 outline-none focus:border-[#b08d57] transition-colors">
                ...
                <div>
                  <label className="block text-[9px] tracking-[.2em] uppercase text-[#52504c] mb-2">
                    Reward URL <span className="text-[#2e2c29] normal-case">shown after email submitted</span>
                  </label>
                ...
                <div>
                  <label className="block text-[9px] tracking-[.2em] uppercase text-[#52504c] mb-2">
                    Specific post URL <span className="text-[#2e2c29] normal-case">leave blank = any post</span>
                  </label>
                ...
                <div className="col-span-2">
                  <label className="flex items-center gap-3 cursor-pointer">
                    <div onClick={() => setForm(p => ({ ...p, follow_required: !p.follow_required }))}
                      className={`w-8 h-4 rounded-full transition-colors relative ${form.follow_required ? 'bg-[#b08d57]' : 'bg-white/10'}`}>
                      <div className={`absolute top-0.5 w-3 h-3 rounded-full bg-white transition-all ${form.follow_required ? 'left-4' : 'left-0.5'}`} />
                    </div>
                    <span className="text-[10px] tracking-[.08em] text-[#8a8780]">Require follow before download</span>
                  </label>
                </div>
              </div>
              <div className="flex gap-3">
                <button onClick={createAutomation} disabled={saving || !form.campaign_name || !form.dm_message}
                  className="text-[10px] tracking-[.16em] uppercase bg-[#b08d57] text-[#070706] px-6 py-2.5 hover:bg-[#c9a46e] transition-colors disabled:opacity-40 flex items-center gap-2">
                  {saving && <div className="w-2 h-2 border border-current border-t-transparent rounded-full animate-spin" />}
                  {saving ? 'Creating...' : 'Create campaign →'}
                </button>
                <button onClick={() => setCreating(false)} className="text-[10px] tracking-[.16em] uppercase border border-white/13 text-[#52504c] px-6 py-2.5 hover:border-white/25 transition-colors">
                  Cancel
                </button>
              </div>
            </div>
            ...
            <div className="p-8">
              <div className="flex items-start justify-between mb-6">
                <div>
                  ...
                  <div className="flex items-center gap-3 mt-1">
                    <span className={`text-[9px] tracking-[.14em] uppercase px-2 py-0.5 ${selectedAuto.enabled ? 'bg-[#3d6b4a]/20 text-[#3d6b4a]' : 'bg-white/5 text-[#52504c]'}`}>
                      {selectedAuto.enabled ? 'live' : 'paused'}
                    </span>
                    <span className="text-[10px] text-[#52504c]">{selectedAuto.sent_count || 0} DMs sent · {selectedAuto.lead_count || 0} leads</span>
                    {selectedAuto.follow_required && (
                      <span className="text-[9px] tracking-[.1em] uppercase text-[#b08d57]/60">follow-gated</span>
                    )}
                  </div>
                </div>
                <div className="flex items-center gap-3">
                  <button onClick={() => toggleAutomation(selectedAuto.id, !selectedAuto.enabled)}
                    className="text-[9px] tracking-[.14em] uppercase text-[#52504c] hover:text-[#8a8780] transition-colors border border-white/10 px-3 py-1.5">
              ...
              <div className="grid grid-cols-2 gap-3 mb-6">
                <div className="bg-[#0e0d0b] border border-white/7 p-4">
                  <div className="text-[9px] tracking-[.18em] uppercase text-[#52504c] mb-2">Trigger keyword</div>
                  <div className="text-[12px] text-[#f0ebe2]">{selectedAuto.trigger_keyword || <span className="text-[#52504c] italic">any comment</span>}</div>
                </div>
                <div className="bg-[#0e0d0b] border border-white/7 p-4">
                  <div className="text-[9px] tracking-[.18em] uppercase text-[#52504c] mb-2">Reward type</div>
                  <div className="text-[12px] text-[#f0ebe2] capitalize">{selectedAuto.reward_type}</div>
                </div>
                <div className="bg-[#0e0d0b] border border-white/7 p-4 col-span-2">
                  <div className="text-[9px] tracking-[.18em] uppercase text-[#52504c] mb-2">DM message</div>
              ...
              <div className="border-t border-white/7 pt-6">
                <div className="flex items-center justify-between mb-4">
                  <div className="text-[10px] tracking-[.22em] uppercase text-[#b08d57]">Leads captured — {leads.length}</div>
                  {leads.length > 0 && (
                    <button onClick={exportCSV} className="text-[9px] tracking-[.14em] uppercase text-[#52504c] hover:text-[#8a8780] transition-colors border border-white/10 px-3 py-1.5">
                      Export CSV →
                    </button>
                  )}
                </div>
                ...
                {loadingLeads ? (
                  <div className="flex items-center gap-2 py-6 text-[10px] tracking-[.1em] text-[#52504c]">
                  ...
                  <div className="border border-dashed border-white/10 p-8 text-center">
                    <div className="text-[10px] tracking-[.08em] text-[#52504c]">No leads yet — activate the campaign and post</div>
                  </div>
                ) : (
                  <div className="border border-white/7">
                    <div className="grid grid-cols-5 gap-0 border-b border-white/7 px-4 py-2 bg-white/3">
                      {['Instagram', 'Email', 'Followers', 'Comment', 'Date'].map(h => (
                        <div key={h} className="text-[8px] tracking-[.2em] uppercase text-[#52504c]">{h}</div>
                      ))}
                    </div>
                    {leads.map(lead => (
                      <div key={lead.id} className="grid grid-cols-5 gap-0 border-b border-white/5 px-4 py-3 hover:bg-white/3 transition-colors last:border-0">
                        <div className="text-[10px] tracking-[.04em]">{lead.username ? `@${lead.username}` : <span className="text-[#52504c]">—</span>}</div>
                        <div className="text-[10px] tracking-[.04em] text-[#8a8780] truncate">{lead.email || <span className="text-[#52504c]">—</span>}</div>
                        <div className="text-[10px] text-[#8a8780]">{lead.follower_count ? lead.follower_count.toLocaleString() : '—'}</div>
                        <div className="text-[9px] text-[#52504c] truncate italic">"{(lead.comment_text || '').slice(0, 30)}"</div>
                        <div className="text-[9px] text-[#52504c]">
          ...
          {!selectedAuto && !creating && (
            <div className="flex items-center justify-center h-full text-center px-8">
              <div>
                <div className="text-[10px] tracking-[.1em] text-[#52504c] mb-4">
                  Comment → DM → email capture → leads database
                </div>
                <button onClick={() => setCreating(true)}
                  className="text-[10px] tracking-[.18em] uppercase text-[#b08d57] hover:opacity-70 transition-opacity">
