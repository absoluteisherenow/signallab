  ...
  return (
    <div style={{ background: COLORS.bg, color: COLORS.text, fontFamily: "'DM Mono', monospace", minHeight: '100vh' }}>
      ...
      <nav style={{ position: 'fixed', top: 0, left: 0, right: 0, zIndex: 100, background: `rgba(7,7,6,0.92)`, borderBottom: `1px solid ${COLORS.border}`, padding: '20px 48px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', backdropFilter: 'blur(8px)' }}>
        <Link href="/"><div style={{ fontFamily: "'Unbounded', sans-serif", fontSize: '11px', fontWeight: 300, letterSpacing: '0.2em', color: COLORS.gold, cursor: 'pointer' }}>SIGNAL LAB OS</div></Link>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '32px', marginBottom: '80px' }}>
            {TIERS.map((tier) => (
              <div key={tier.name} style={{ background: tier.highlight ? `linear-gradient(135deg, ${COLORS.panel} 0%, #1a1310 100%)` : COLORS.panel, border: `2px solid ${tier.highlight ? tier.color : COLORS.border}`, padding: '40px 28px', position: 'relative', transform: tier.highlight ? 'scale(1.04)' : 'scale(1)' }}>
                {tier.highlight && <div style={{ position: 'absolute', top: '-12px', left: '50%', transform: 'translateX(-50%)', background: tier.color, color: COLORS.bg, padding: '5px 14px', fontSize: '10px', letterSpacing: '0.1em', fontWeight: 'bold' }}>MOST POPULAR</div>}
                ...
                <div style={{ marginBottom: '28px' }}>
                  ...
                  <div style={{ fontSize: '10px', color: COLORS.textDimmer, letterSpacing: '0.08em', marginTop: '6px' }}>Cancel anytime</div>
                </div>
                <button onClick={() => router.push('/login')} style={{ width: '100%', background: tier.highlight ? tier.color : 'transparent', color: tier.highlight ? COLORS.bg : tier.color, border: `1px solid ${tier.color}`, padding: '12px 24px', fontSize: '10px', letterSpacing: '0.12em', textTransform: 'uppercase', fontWeight: 'bold', cursor: 'pointer', marginBottom: '24px' }}>Get Access →</button>
      ...
      <section style={{ padding: '80px 48px', borderBottom: `1px solid ${COLORS.border}` }}>
        <div style={{ maxWidth: '900px', margin: '0 auto' }}>
          ...
          <div style={{ fontSize: '10px', color: COLORS.textDimmer, marginTop: '16px', textAlign: 'center' }}>*at 20 shows/year on Artist tier (£59/mo)</div>
        </div>
      </section>
      ...
      {/* FOOTER */}
      <footer style={{ padding: '40px 48px', borderTop: `1px solid ${COLORS.border}`, display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '28px', fontSize: '10px', color: COLORS.textDimmer, letterSpacing: '0.08em' }}>
        <div style={{ fontFamily: "'Unbounded', sans-serif", fontSize: '10px', fontWeight: 300, letterSpacing: '0.12em' }}>SIGNAL LAB OS</div>
