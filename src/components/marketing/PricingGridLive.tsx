'use client'
// Live variant of PricingGrid — same visual design but with CheckoutButton
// CTAs that hit /api/billing/checkout. Used on /pricing.
//
// The static PricingGrid (with #waitlist anchor) is kept for the marketing
// homepage during private beta.
          ...
          >
            ...
            <div className="mt-8 flex items-baseline gap-2">
              <div
                ...
                style={{ fontFamily: DISPLAY, fontSize: 'clamp(64px, 7vw, 92px)', color: BRT.ink }}
              >
                {tier.price}
              </div>
            ...
            <p className="mt-6 font-mono text-[13px] leading-[1.7] min-h-[56px]" style={{ color: BRT.inkSoft }}>
              {tier.whoFor}
            </p>

            <div className="mt-5 pl-5 py-2 text-[13px] leading-[1.6]" style={{ borderLeft: `2px solid ${BRT.red}`, color: BRT.ink, fontFamily: DISPLAY }}>
              {tier.wowAnchor}
            </div>

            <ul className="mt-7 space-y-3 font-mono text-[12.5px] leading-[1.65] flex-1" style={{ color: BRT.inkSoft }}>
              {tier.features.map(f => (
            ...
            <div className="mt-8 pt-6 grid grid-cols-2 gap-x-3 gap-y-3 font-mono" style={{ borderTop: `1px solid ${BRT.divide}` }}>
              {tier.limits.map(l => (
